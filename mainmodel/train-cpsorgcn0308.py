...
from keras.layers import Permute, Dense, TimeDistributed, Conv2D, MaxPooling2D, Multiply, Dropout, Flatten, Reshape, \
    LSTM, UpSampling2D, concatenate, Input, Bidirectional, Lambda, RepeatVector,BatchNormalization
...
H_phy = X_in_phy
for ii in range(5):
    H_phy = GraphConvolution(64 // (2 ** ii), support, activation='relu', kernel_regularizer=l2(5e-4))([H_phy] + [[Gn_phy]])
...
H_psy = X_in_psy
for jj in range(5):
    H_psy = GraphConvolution(64 // (2 ** jj), support, activation='relu', kernel_regularizer=l2(5e-4))([H_psy] + [[Gn_psy]])
...
Y = concatenate([g_reshape_phy, g_reshape_psy], axis=-1)

bilstm = LSTM(128, dropout=0.1, return_sequences=True,input_shape=(n_step_in, n_node_in + psyn_node_in))(Y)
...
dense_out = Dense(n_step_out*n_node_out*n_feature_out)(dropout)
dense_out = Reshape((n_step_out,n_node_out,n_feature_out))(dense_out)
inputss = model1.input + model2.input
model = Model(inputs=inputss, outputs=dense_out)
